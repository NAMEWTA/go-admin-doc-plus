...
import { getSetConfig, updateSetConfig } from '../../api/sys-config'
import { getApiBaseUrl, getSessionToken, refreshSystemConfig } from '../../app-core'
...
// ── Logo upload ───────────────────────────────────────────────────
const uploadHeaders = { Authorization: 'Bearer ' + getSessionToken() }
const uploadAction = getApiBaseUrl() + '/api/v1/public/uploadFile'
...
const onLogoUploaded: UploadProps['onSuccess'] = response => {
  form.value.sys_app_logo = getApiBaseUrl() + response.data.full_path
}
</script>
