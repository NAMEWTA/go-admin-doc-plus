...
import { asReportedError, notifySuccess } from '../../../app-core/src'
import type { ReportedError } from '../../../app-core/src'
import { ElMessageBox } from 'element-plus'
import type { Id } from '../../../app-core/src'
...
export function useRemove(options: UseRemoveOptions): UseRemoveReturn {
  ...
  const remove = async(target: Id | Id[] | undefined): Promise<boolean> => {
    ...
    try {
      ...
      removing.value = true
      await api(ids)
      if (successMessage !== null) notifySuccess(successMessage ?? '删除成功')
      await onSuccess?.(ids)
      return true
    } catch(error) {
