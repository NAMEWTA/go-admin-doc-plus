...
} from '../packages/contracts/generate.mjs'

const UI = join(dirname(fileURLToPath(import.meta.url)), '..')
const ADMIN_SRC = join(UI, 'apps/admin/src')
const DOMAINS = join(UI, 'domains')
const GO = process.env.GO_ADMIN_PATH ?? join(UI, '..', 'go-admin-plus')
...
const contracts = readGeneratedContracts()
if (contracts.generated === null) {
  const where = `no canonical OpenAPI at ${contracts.openapiPath}`
  if (required) {
    console.error(`cannot generate the api contract: ${where}`)
    process.exit(1)
  }
  console.log(`skipped generated contract: ${where}`)
} else if (write) {
  writeFileSync(generatedContractsPath, contracts.generated)
  console.log(`generated contracts from ${Object.keys(contracts.document.paths).length} OpenAPI paths`)
} else if (!existsSync(generatedContractsPath) || readFileSync(generatedContractsPath, 'utf8') !== contracts.generated) {
  console.error('generated API contracts are stale; run pnpm --filter @go-admin/contracts generate')
  process.exit(1)
} else {
  console.log(`generated API contracts are current: ${Object.keys(contracts.document.paths).length} OpenAPI paths`)
}
...
/** Every .vue under a page root, recursively. */
...
const pageRoots = [join(ADMIN_SRC, 'views')]
for (const entry of readdirSync(DOMAINS, { withFileTypes: true })) {
  if (!entry.isDirectory()) continue
  const pages = join(DOMAINS, entry.name, 'src/pages')
  if (existsSync(pages)) pageRoots.push(pages)
}
...
const migratedPages = () => {
  ...
  for (const root of pageRoots) {
    for (const path of vueFiles(root)) {
      ...
      found[path.slice(UI.length + 1)] = MODEL_ALIASES[name] ?? name
    }
  }
  return found
}
 ...
 */
const OPTIONS_PAGES = {
  'domains/tools/src/pages/gen/editTable.vue': ['SysTables', 'SysColumns'],
  'domains/tools/src/pages/gen/genInfoForm.vue': ['SysTables'],
  'domains/tools/src/pages/gen/basicInfoForm.vue': ['SysTables']
}
...
for (const [page, model] of Object.entries(PAGES)) {
  const text = readFileSync(join(UI, page), 'utf8')
  const fields = fieldsOf(model)
 ...
 */
const declaredTypes = readFileSync(join(ADMIN_SRC, 'types/admin.ts'), 'utf8')
