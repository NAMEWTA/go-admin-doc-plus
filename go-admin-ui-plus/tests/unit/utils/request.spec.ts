...
import { createDesktopTransport, normalizeRequest } from '@/utils/request'
...
describe('request boundary normalization', () => {
  it('removes a reactive plain-object wrapper before crossing into ApiClient', () => {
    const source = new Proxy(
      { username: 'admin', password: '123456', code: '1' },
      { get: (target, key, receiver) => Reflect.get(target, key, receiver) }
    )

    const normalized = normalizeRequest({ url: '/api/v1/login', method: 'post', data: source })

    expect(normalized.body).toEqual(source)
    expect(normalized.body).not.toBe(source)
  })

  it('preserves non-plain request bodies', () => {
    const form = new FormData()
    expect(normalizeRequest({ url: '/api/v1/upload', data: form }).body).toBe(form)
  })

  it('sends Desktop JSON through native fetch with query, headers and timeout signal', async() => {
    const fetcher = vi.fn<typeof fetch>(async() => new Response(
      JSON.stringify({ code: 200, data: null, msg: 'ok' }),
      { status: 200 }
    ))
    const transport = createDesktopTransport(fetcher)

    await expect(transport.send({
      url: 'http://127.0.0.1:43125/api/v1/login',
      method: 'post',
      query: { source: 'desktop' },
      body: { username: 'admin' },
      headers: { 'X-Go-Admin-Launch-Token': 'secret' },
      timeoutMs: 1_000
    })).resolves.toEqual({ status: 200, data: { code: 200, data: null, msg: 'ok' }})

    expect(fetcher).toHaveBeenCalledWith(
      'http://127.0.0.1:43125/api/v1/login?source=desktop',
    ...
    expect(headers.get('X-Go-Admin-Launch-Token')).toBe('secret')
  })
})
