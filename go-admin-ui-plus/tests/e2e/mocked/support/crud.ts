...
const canonicalEnvelope = (body: unknown) => {
  if (!body || typeof body !== 'object' || typeof (body as { code?: unknown }).code !== 'number') return body
  const envelope = body as Record<string, unknown> & { code: number }
  return {
    msg: 'ok',
    ...(envelope.code === 200 ? { data: null } : {}),
    ...envelope
  }
}
...
export const json = (body: unknown) => ({
  ...
  body: JSON.stringify(canonicalEnvelope(body))
})
