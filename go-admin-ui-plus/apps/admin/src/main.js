...
import { domainApiClient } from '@/utils/request'
import {
  configureAppHostPorts,
  configureDictionaryLoader,
  configureDomainApiClient,
  configureSuccessNotifier
...
const iconNames = Object.keys(iconModules).map(path => path.match(/\/([^/]+)\.svg$/)?.[1]).filter(Boolean)

configureDomainApiClient(domainApiClient)
configureDictionaryLoader(async type => (await getDicts(type)).data ?? [])
configureSuccessNotifier(msgSuccess)
configureAppHostPorts({
  refreshSystemConfig: () => useSystemStore().settingDetail(),
  closeCurrentView: async nextPath => {
    useTagsViewStore().delView(router.currentRoute.value)
    await router.push(nextPath || '/index')
  },
  getSessionToken: () => getToken() || '',
  getApiBaseUrl: () => process.env.VUE_APP_BASE_API || '',
  listIcons: () => iconNames
})
