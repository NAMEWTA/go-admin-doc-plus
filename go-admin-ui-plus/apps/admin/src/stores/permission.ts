...
import UnavailableRoute from '../../../../packages/app-core/src/UnavailableRoute.vue'
import { nameRouteComponent } from '../../../../packages/app-core/src'
import type { BackendMenu } from '../../../../packages/app-core/src'

export type { BackendMenu } from '../../../../packages/app-core/src'
...
export const loadView = (view: string, name?: string) => {
  const domainRoute = domainRegistry.resolve({ legacyComponent: view })
  if (domainRoute.status === 'resolved') {
    return nameRouteComponent(domainRoute.route.load, name)
  }
...
const unavailableView = (name?: string) => nameRouteComponent(
  () => Promise.resolve(UnavailableRoute),
  name
)

const resolveMenuView = (item: BackendMenu) => {
  const resolution = domainRegistry.resolve({
    routeKey: item.routeKey,
    legacyComponent: item.component
  })
  if (resolution.status === 'resolved') {
    return nameRouteComponent(resolution.route.load, item.menuName)
  }
  if (item.routeKey) {
    console.warn('Unknown menu routeKey', { routeKey: item.routeKey, component: item.component })
    return unavailableView(item.menuName)
  }
  try {
    return loadView(item.component, item.menuName)
  } catch {
    console.warn('Unknown legacy menu component', { component: item.component })
    return unavailableView(item.menuName)
  }
}
...
export function generaMenu(routes: AppRoute[], data: BackendMenu[]) {
  data.forEach(item => {
    const menu = {
      path: item.path,
      component: item.component === 'Layout' ? Layout : resolveMenuView(item),
