...
export const createWebGeneratorClient = (fetcher: typeof fetch = fetch, baseUrl = '/api'): GeneratorClient => {
  let csrf = '', classified: GeneratorFailure | null = null, classifiedTraceId: string | undefined
  ...
  const fail = (error: unknown): never => {
    ...
    const traceId = classifiedTraceId ?? (error instanceof GeneratorRequestError ? error.traceId : undefined)
    classified = null; classifiedTraceId = undefined
    throw new GeneratorRequestError(category, traceId)
  }
