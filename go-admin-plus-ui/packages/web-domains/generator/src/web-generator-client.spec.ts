...
describe('web generator client', () => {
  it('carries CSRF only on mutations and classifies gate failures', async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(json(200, [], { 'X-CSRF-Token': 'c'.repeat(43) })).mockResolvedValueOnce(json(422, { category: 'validation', code: 'OUTPUT_GATE_FAILED', traceId: 'trace_gate_123' }))
    const client = createWebGeneratorClient(fetcher, 'https://generator.test/api')
    await client.listTables()
    await expect(client.write('a'.repeat(64))).rejects.toMatchObject({ category: 'gate', traceId: 'trace_gate_123' })
