...
<template>
  <section class="demo-products" aria-labelledby="demo-products-title">
    ...
    <div v-if="formOpen && canWrite" class="management-dialog-backdrop" @click.self="closeForm" @keydown.esc="closeForm"><form class="management-dialog demo-products__form" role="dialog" aria-modal="true" :aria-labelledby="editing ? 'edit-product-title' : 'create-product-title'" @submit.prevent="save"><header class="management-dialog__header"><h2 :id="editing ? 'edit-product-title' : 'create-product-title'">{{ editing ? '修改产品' : '新增产品' }}</h2><button type="button" aria-label="关闭" :disabled="controller.busy" @click="closeForm">×</button></header><div class="management-dialog__body"><p v-if="formError" role="alert">{{ formError }}</p><label>SKU<input v-model="form.sku" name="sku" aria-label="SKU" autofocus required minlength="3" maxlength="32" pattern="[A-Za-z0-9][A-Za-z0-9_-]{2,31}"></label><label>名称<input v-model="form.name" name="name" aria-label="名称" required minlength="3" maxlength="120"></label><label>描述<textarea v-model="form.description" name="description" aria-label="描述" maxlength="500"></textarea></label><label>价格（分）<input v-model.number="form.priceCents" name="priceCents" aria-label="价格（分）" type="number" min="0" max="100000000" required></label><label>状态<select v-model="form.status" name="status"><option value="active">启用</option><option value="inactive">停用</option></select></label></div><footer class="management-dialog__footer"><button type="button" :disabled="controller.busy" @click="closeForm">取消</button><button type="submit" :disabled="blocked">保存</button></footer></form></div>
  </section>
</template>
