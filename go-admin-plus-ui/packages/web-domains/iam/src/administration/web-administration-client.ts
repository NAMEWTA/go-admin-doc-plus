...
export const createWebAdministrationClient = (fetcher: typeof fetch = fetch, baseUrl = '/api'): AdministrationClient => {
  ...
  return {
    ...
    deleteUser: (userId) => serialized(async () => { const result = await contract.DELETE('/iam/administration/users/{userId}', { params: { path: { userId } } }); complete(result.error) }),
    deleteUsers: (userIds) => serialized(async () => { const result = await contract.POST('/iam/administration/users/batch-delete', { body: { userIds: [...userIds] } }); complete(result.error) }),
