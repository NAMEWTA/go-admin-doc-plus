...
export const createWebAdministrationClient = (fetcher: typeof fetch = fetch, baseUrl = '/api'): AdministrationClient => {
  ...
  return {
    ...
    setUserOrganization: (userId, body) => serialized(async () => { const result = await contract.PUT('/iam/administration/users/{userId}/organization', { params: { path: { userId } }, body }); complete(result.error) }),
    ...
    setRoleDataScope: (roleId, body) => serialized(async () => { const result = await contract.PUT('/iam/administration/roles/{roleId}/data-scope', { params: { path: { roleId } }, body }); complete(result.error) }),
