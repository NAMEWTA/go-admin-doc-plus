  ...
  <main class="administration-page">
    ...
    <section v-if="tab === 'users' && controller.can('iam.users.read')" aria-labelledby="users-heading">
      ...
      <form class="toolbar" data-testid="user-search" @submit.prevent="search">
        ...
        <button v-if="controller.can('iam.users.delete')" type="button" data-testid="delete-selected-users" :disabled="users.selectedKeys.length === 0 || controller.deleteUsers.busy || mutationBlocked" @click="run(() => controller.deleteUsers.run(users.selectedKeys))">Delete selected</button>
      </form>
      ...
      <form v-if="editedUser && controller.can('iam.roles.assign')" class="editor" data-testid="assign-user-roles" @submit.prevent="saveUserRoles"><h3>Assign roles</h3><label v-for="item in roles" :key="item.id" class="choice"><input v-model="assignedRoles" type="checkbox" :value="item.id" :data-role-key="item.key">{{ item.name }}</label><button type="submit" :disabled="mutationBlocked">Save role assignments</button></form>
    ...
    <section v-else-if="tab === 'roles' && controller.can('iam.roles.read')" aria-labelledby="roles-heading">
      ...
      <table><thead><tr><th>Key</th><th>Name</th><th>Data scope</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="item in roles" :key="item.id" :data-row-key="item.key"><td>{{ item.key }}</td><td>{{ item.name }}</td><td>{{ item.dataScope }}</td><td>{{ item.enabled ? 'Enabled' : 'Disabled' }}</td><td><button v-if="controller.can('iam.roles.write')" type="button" data-action="edit" @click="editRole(item)">Edit</button><button v-if="controller.can('iam.roles.delete')" type="button" data-action="delete" :disabled="item.protected || mutationBlocked" @click="run(() => controller.deleteRole(item.id))">Delete</button></td></tr></tbody></table>
