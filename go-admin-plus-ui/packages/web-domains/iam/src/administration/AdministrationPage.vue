...
<template>
  <main class="administration-page ga-work-page">
    ...
    <section v-if="view === 'users' && can('iam.users.read')" aria-labelledby="users-heading">
      ...
      <form class="toolbar" data-testid="user-search" @submit.prevent="search">
        ...
        <button v-if="can('iam.users.write')" type="button" data-testid="open-create-user" @click="openCreateUser"><PlusIcon :size="15" aria-hidden="true" />新增</button>
      </form>
      <div class="table-scroll">
        <table><thead><tr><th scope="col">用户名</th><th scope="col">姓名</th><th scope="col">邮箱</th><th scope="col">状态</th><th scope="col">操作</th></tr></thead>
          <tbody><tr v-for="row in users.rows" :key="row.id" :data-row-key="row.username"><td><strong>{{ row.username }}</strong></td><td>{{ row.displayName }}</td><td>{{ row.email }}</td><td><StatusTag :tone="row.disabled ? 'neutral' : 'success'" :label="row.disabled ? '停用' : '启用'" /></td><td><button v-if="can('iam.users.write')" type="button" data-action="edit" @click="editUser(row)"><PencilIcon :size="14" aria-hidden="true" />编辑</button><button v-if="can('iam.users.write')" type="button" data-action="toggle" :disabled="mutationBlocked" @click="toggleUser(row)"><PowerIcon :size="14" aria-hidden="true" />{{ row.disabled ? '启用' : '停用' }}</button><button v-if="can('iam.users.delete')" type="button" data-action="delete" :disabled="mutationBlocked" @click="openDeletion(row)"><Trash2Icon :size="14" aria-hidden="true" />删除</button></td></tr></tbody>
        </table>
      </div>
    ...
    <section v-else-if="view === 'roles' && can('iam.roles.read')" aria-labelledby="roles-heading">
      ...
      <div class="toolbar management-toolbar"><button v-if="can('iam.roles.write')" type="button" data-testid="open-create-role" @click="openCreateRole"><PlusIcon :size="15" aria-hidden="true" />新增</button></div>
      <div class="table-scroll"><table><thead><tr><th>角色标识</th><th>角色名称</th><th>数据范围</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="item in roles" :key="item.id" :data-row-key="item.key"><td>{{ item.key }}</td><td>{{ item.name }}</td><td>{{ item.dataScope === 'all' ? '全部' : '本人' }}</td><td><StatusTag :tone="item.enabled ? 'success' : 'neutral'" :label="item.enabled ? '启用' : '停用'" /></td><td><button v-if="can('iam.roles.write')" type="button" data-action="edit" @click="editRole(item)"><PencilIcon :size="14" aria-hidden="true" />编辑</button><button v-if="can('iam.roles.delete')" type="button" data-action="delete" :disabled="item.protected || mutationBlocked" @click="run(() => controller.deleteRole(item.id))"><Trash2Icon :size="14" aria-hidden="true" />删除</button></td></tr></tbody></table></div>
    ...
    <section v-else-if="view === 'menus' && can('iam.menus.read')" aria-labelledby="menus-heading">
      ...
      <div class="toolbar management-toolbar"><button v-if="can('iam.menus.write')" type="button" data-testid="open-create-menu" @click="openCreateMenu"><PlusIcon :size="15" aria-hidden="true" />新增</button></div>
