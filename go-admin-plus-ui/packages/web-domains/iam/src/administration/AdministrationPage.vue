...
<template>
  <main class="administration-page">
    ...
    <nav class="tabs" aria-label="IAM administration views">
      <button v-for="value in (['users', 'roles', 'menus'] as const).filter((item) => can(`iam.${item}.read`))" :key="value" type="button" :aria-pressed="tab === value" @click="tab = value">{{ value }}</button>
    </nav>
    ...
    <section v-if="tab === 'users' && can('iam.users.read')" aria-labelledby="users-heading">
      ...
      <form class="toolbar" data-testid="user-search" @submit.prevent="search">
        ...
        <button v-if="can('iam.users.delete')" type="button" data-testid="delete-selected-users" :disabled="users.selectedKeys.length === 0 || controller.deleteUsers.busy || mutationBlocked" @click="run(() => controller.deleteUsers.run(users.selectedKeys))">Delete selected</button>
      </form>
      <table><thead><tr><th scope="col">Select</th><th scope="col">Username</th><th scope="col">Name</th><th scope="col">Email</th><th scope="col">Status</th><th scope="col">Actions</th></tr></thead>
        <tbody><tr v-for="row in users.rows" :key="row.id" :data-row-key="row.username"><td><input v-if="can('iam.users.delete')" type="checkbox" :checked="users.selectedKeys.includes(row.id)" :aria-label="`Select ${row.username}`" @change="selectUser(($event.target as HTMLInputElement).checked, row)"></td><td>{{ row.username }}</td><td>{{ row.displayName }}</td><td>{{ row.email }}</td><td>{{ row.disabled ? 'Disabled' : 'Enabled' }}</td><td><button v-if="can('iam.users.write')" type="button" data-action="edit" @click="editUser(row)">Edit</button><button v-if="can('iam.users.write')" type="button" data-action="toggle" :disabled="mutationBlocked" @click="toggleUser(row)">{{ row.disabled ? 'Enable' : 'Disable' }}</button><button v-if="can('iam.users.delete')" type="button" data-action="delete" :disabled="controller.deleteUsers.busy || mutationBlocked" @click="deleteUser(row)">Delete</button></td></tr></tbody>
      </table>
      ...
      <form v-if="can('iam.users.write')" class="editor" data-testid="create-user" @submit.prevent="submitUser"><h3>Create user</h3><label>Username<input name="username" v-model.trim="user.username" required minlength="3" maxlength="64"></label><label>Display name<input name="displayName" v-model.trim="user.displayName" required maxlength="80"></label><label>Email<input name="email" v-model.trim="user.email" type="email" required></label><label>Password<input name="password" v-model="user.password" type="password" required minlength="12" autocomplete="new-password"></label><button type="submit" :disabled="controller.createUser.busy || mutationBlocked">Create user</button></form>
      <form v-if="editedUser && can('iam.users.write')" class="editor" data-testid="edit-user" @submit.prevent="saveUser"><h3>Edit {{ editedUser.username }}</h3><label>Display name<input name="displayName" v-model.trim="editedUser.displayName" required maxlength="80"></label><label>Email<input name="email" v-model.trim="editedUser.email" type="email" required></label><button type="submit" :disabled="mutationBlocked">Save user</button></form>
      <form v-if="editedUser && can('iam.roles.assign')" class="editor" data-testid="assign-user-roles" @submit.prevent="saveUserRoles"><h3>Assign roles</h3><label v-for="item in roles" :key="item.id" class="choice"><input v-model="assignedRoles" type="checkbox" :value="item.id" :data-role-key="item.key">{{ item.name }}</label><button type="submit" :disabled="mutationBlocked">Save role assignments</button></form>
      <form v-if="editedUser && can('iam.users.reset-password')" class="editor" data-testid="reset-user-password" @submit.prevent="resetPassword"><h3>Reset password</h3><label>Replacement password<input name="password" v-model="replacementPassword" type="password" required minlength="12" autocomplete="new-password"></label><button type="submit" :disabled="mutationBlocked">Reset password</button></form>
    </section>

    <section v-else-if="tab === 'roles' && can('iam.roles.read')" aria-labelledby="roles-heading">
      <h2 id="roles-heading">Roles</h2>
      <table><thead><tr><th>Key</th><th>Name</th><th>Data scope</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="item in roles" :key="item.id" :data-row-key="item.key"><td>{{ item.key }}</td><td>{{ item.name }}</td><td>{{ item.dataScope }}</td><td>{{ item.enabled ? 'Enabled' : 'Disabled' }}</td><td><button v-if="can('iam.roles.write')" type="button" data-action="edit" @click="editRole(item)">Edit</button><button v-if="can('iam.roles.delete')" type="button" data-action="delete" :disabled="item.protected || mutationBlocked" @click="run(() => controller.deleteRole(item.id))">Delete</button></td></tr></tbody></table>
      <form v-if="can('iam.roles.write')" class="editor" data-testid="create-role" @submit.prevent="submitRole"><h3>Create role</h3><label>Key<input name="key" v-model.trim="role.key" required minlength="3" maxlength="64" pattern="[a-z0-9][a-z0-9_-]*"></label><label>Name<input name="name" v-model.trim="role.name" required maxlength="100"></label><label>Data scope<select name="dataScope" v-model="role.dataScope"><option value="self">Self</option><option value="all">All</option></select></label><button type="submit" :disabled="controller.createRole.busy || mutationBlocked">Create role</button></form>
      <form v-if="editedRole && can('iam.roles.write')" class="editor" data-testid="edit-role" @submit.prevent="saveRole"><h3>Edit {{ editedRole.key }}</h3><label>Name<input name="name" v-model.trim="editedRole.name" required maxlength="100"></label><label>Data scope<select name="dataScope" v-model="editedRole.dataScope"><option value="self">Self</option><option value="all">All</option></select></label><label class="choice"><input name="enabled" v-model="editedRole.enabled" type="checkbox">Enabled</label><button type="submit" :disabled="editedRole.protected || mutationBlocked">Save role</button></form>
      <form v-if="editedRole && can('iam.roles.assign')" class="editor" data-testid="assign-role-grants" @submit.prevent="saveRoleGrants"><h3>Permission and menu grants</h3><fieldset><legend>Permission Codes</legend><label v-for="item in controller.permissions()" :key="item.code" class="choice"><input v-model="grantedPermissions" type="checkbox" :value="item.code" :data-permission-code="item.code">{{ item.name }}</label></fieldset><fieldset><legend>Menus</legend><label v-for="item in menus" :key="item.id" class="choice"><input v-model="grantedMenus" type="checkbox" :value="item.id" :data-menu-key="item.key">{{ item.label }}</label></fieldset><button type="submit" :disabled="editedRole.protected || mutationBlocked">Save grants</button></form>
    </section>

    <section v-else-if="tab === 'menus' && can('iam.menus.read')" aria-labelledby="menus-heading">
      <h2 id="menus-heading">Menus</h2>
      <table><thead><tr><th>Key</th><th>Label</th><th>Path</th><th>Permission</th><th>Actions</th></tr></thead><tbody><tr v-for="item in menus" :key="item.id" :data-row-key="item.key"><td>{{ item.key }}</td><td>{{ item.label }}</td><td>{{ item.path }}</td><td>{{ item.permissionCode }}</td><td><button v-if="can('iam.menus.write')" type="button" data-action="edit" @click="editMenu(item)">Edit</button><button v-if="can('iam.menus.delete')" type="button" data-action="delete" :disabled="item.protected || mutationBlocked" @click="run(() => controller.deleteMenu(item.id))">Delete</button></td></tr></tbody></table>
      <form v-if="can('iam.menus.write')" class="editor" data-testid="create-menu" @submit.prevent="submitMenu"><h3>Create menu</h3><label>Key<input name="key" v-model.trim="menu.key" required minlength="3" maxlength="64" pattern="[a-z0-9][a-z0-9_-]*"></label><label>Label<input name="label" v-model.trim="menu.label" required maxlength="80"></label><label>Path<input name="path" v-model.trim="menu.path" required pattern="/[a-z0-9/_-]+"></label><label>Permission code<input name="permissionCode" v-model.trim="menu.permissionCode" required minlength="3"></label><label>Order<input name="sortOrder" v-model.number="menu.sortOrder" type="number" min="0" max="100000"></label><button type="submit" :disabled="controller.createMenu.busy || mutationBlocked">Create menu</button></form>
      <form v-if="editedMenu && can('iam.menus.write')" class="editor" data-testid="edit-menu" @submit.prevent="saveMenu"><h3>Edit {{ editedMenu.key }}</h3><label>Label<input name="label" v-model.trim="editedMenu.label" required maxlength="80"></label><label>Path<input name="path" v-model.trim="editedMenu.path" required pattern="/[a-z0-9/_-]+"></label><label>Permission code<input name="permissionCode" v-model.trim="editedMenu.permissionCode" required minlength="3"></label><label>Order<input name="sortOrder" v-model.number="editedMenu.sortOrder" type="number" min="0" max="100000"></label><button type="submit" :disabled="editedMenu.protected || mutationBlocked">Save menu</button></form>
    </section>
