...
<template>
  <main class="scheduler-page">
    ...
    <section v-if="view === 'definitions' && can('scheduler.definitions.read')">
      ...
      <div class="pagination" data-testid="scheduler-definitions-pagination"><span>共 {{ definitions.total }} 条</span><button type="button" :disabled="blocked || definitions.page <= 1" @click="run(() => controller.definitions.setPage(definitions.page - 1))">上一页</button><span>第 {{ definitions.page }} 页</span><label>每页<select :value="definitions.pageSize" :disabled="blocked" @change="run(() => controller.definitions.setPageSize(Number(($event.target as HTMLSelectElement).value)))"><option :value="10">10</option><option :value="20">20</option><option :value="50">50</option></select></label><button type="button" :disabled="blocked || definitions.page * definitions.pageSize >= definitions.total" @click="run(() => controller.definitions.setPage(definitions.page + 1))">下一页</button></div>
