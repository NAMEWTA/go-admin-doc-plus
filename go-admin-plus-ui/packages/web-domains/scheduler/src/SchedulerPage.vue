...
<template>
  <main class="scheduler-page">
    ...
    <section v-if="tab === 'definitions' && can('scheduler.definitions.read')">
      <form class="toolbar" @submit.prevent="searchDefinitions"><label>任务名称<input v-model.trim="search" maxlength="100" placeholder="请输入任务名称"></label><button type="submit">搜索</button><button type="button" @click="search = ''; run(() => controller.definitions.reset())">重置</button></form>
      ...
      <form v-if="can('scheduler.definitions.write')" class="editor" data-testid="scheduler-definition-form" @submit.prevent="submit">
        <h2>{{ edited ? '修改任务' : '新增任务' }}</h2>
        <label>任务名称<input name="name" v-model.trim="form.name" required maxlength="100"></label><label>任务类型<select name="taskType" v-model="form.taskType" required :disabled="Boolean(edited)"><option disabled value="">请选择</option><option v-for="task in taskTypes" :key="task.key" :value="task.key">{{ task.label }}</option></select></label>
        <fieldset><legend>UTC 执行计划</legend><label>分钟<input name="minutes" v-model.trim="form.minutes" required pattern="[0-9, ]+"></label><label>小时<input name="hours" v-model.trim="form.hours" required pattern="[0-9, ]+"></label><label>每月日期<input name="daysOfMonth" v-model.trim="form.daysOfMonth" pattern="[0-9, ]*"></label><label>月份<input name="months" v-model.trim="form.months" required pattern="[0-9, ]+"></label><label>星期<input name="weekdays" v-model.trim="form.weekdays" pattern="[0-9, ]*"></label></fieldset>
        <fieldset v-if="selectedTask" data-testid="scheduler-parameters"><legend>任务参数</legend><label v-for="field in selectedTask.fields" :key="field.name">{{ field.label }}<select v-if="field.allowedValues" :name="field.name" v-model="form.parameters[field.name]" :required="field.required"><option v-for="value in field.allowedValues" :key="value" :value="value">{{ value }}</option></select><input v-else-if="field.kind === 'integer'" :name="field.name" v-model.number="form.parameters[field.name]" type="number" :min="numberValue(field).min" :max="numberValue(field).max" :required="field.required"><input v-else-if="field.kind === 'boolean'" :name="field.name" v-model="form.parameters[field.name]" type="checkbox"><input v-else :name="field.name" v-model="form.parameters[field.name]" maxlength="256" :required="field.required"></label></fieldset>
        <div class="commands"><button type="submit" :disabled="blocked">{{ edited ? '保存' : '新增' }}</button><button v-if="edited" type="button" @click="resetForm">取消</button></div>
      </form>
    </section>
    ...
    <section v-else-if="tab === 'executions' && can('scheduler.executions.read')">
      ...
      <table><thead><tr><th>开始时间</th><th>任务类型</th><th>状态</th><th>计划时间</th><th>错误</th><th>执行器</th></tr></thead><tbody><tr v-for="item in executions.rows" :key="item.id"><td>{{ item.startedAt }}</td><td>{{ item.taskType }}</td><td>{{ item.status }}</td><td>{{ item.scheduledFor }}</td><td>{{ item.errorCode ?? '' }}</td><td>{{ item.executorOwner }}</td></tr></tbody></table>
...
<style scoped>
.scheduler-page { display: grid; gap: 20px; max-width: 1180px; padding: 24px; } h1, h2 { margin: 0; letter-spacing: 0; } section, .editor, fieldset { display: grid; gap: 16px; } .tabs, .toolbar, .pagination, .commands { display: flex; gap: 8px; align-items: end; flex-wrap: wrap; } .tabs button[aria-pressed="true"] { border-bottom-color: var(--ga-brand); color: var(--ga-brand); } .editor { grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); border-top: 1px solid var(--ga-border-light); padding-top: 16px; } .editor h2, .editor fieldset, .commands { grid-column: 1 / -1; } fieldset { grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); border: 1px solid var(--ga-border-light); } label { display: grid; gap: 6px; } input, select, button { min-height: 40px; font: inherit; } input[type="checkbox"] { width: 22px; } button { border: 1px solid var(--ga-border); background: var(--ga-bg-container); padding: 6px 12px; } button:disabled { opacity: .5; } table { width: 100%; border-collapse: collapse; } th, td { border-bottom: 1px solid var(--ga-border-light); padding: 10px; text-align: left; } th { color: var(--ga-text-2); font-size: 14px; } @media (max-width: 700px) { .scheduler-page { padding: 16px; overflow-x: auto; } table { min-width: 840px; } }
</style>
