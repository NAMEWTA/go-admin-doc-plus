...
export const createSettingsController=(client:SettingsClient,confirm:(kind:string)=>Promise<boolean>,capability:SettingsCapabilityPort):SettingsController=>{
  let currentCategory:SettingCategory='business',requestedCategory:SettingCategory=currentCategory,currentDictionary:Dictionary|null=null,requestedDictionary:Dictionary|null=currentDictionary,failure:SettingsFailure|null=null,busy=false,selectionBusy=false,pending:Projection|null=null,categorySelection=0,dictionarySelection=0
  ...
  const mutate=async(target:Projection,permission:SettingsPermissionCode,operation:()=>Promise<void>,confirmation:string):Promise<MutationResult>=>{if(busy||selectionBusy)return'busy';if(pending)return'refresh-failed';if(!visible(permission)){failure='forbidden';return'failed'};busy=true;failure=null;try{if(!await confirm(confirmation))return'cancelled';if(!visible(permission)){failure='forbidden';return'failed'};try{await operation()}catch(error){record(error);return'failed'};pending=target;projection(target).clearSelection();return await refresh(target)}finally{busy=false}}
  const beginSelection=()=>{if(busy||selectionBusy){failure='unavailable';throw new SettingsRequestError('unavailable')}selectionBusy=true}
  return {settings,dictionaries,items,get busy(){return busy||selectionBusy},get pendingRepair(){return pending!==null},failure:()=>failure,can:visible,category:()=>currentCategory,dictionary:()=>currentDictionary,
