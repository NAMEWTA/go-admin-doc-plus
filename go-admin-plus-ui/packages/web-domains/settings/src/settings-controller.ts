...
export const createSettingsController=(client:SettingsClient,confirm:(kind:string)=>Promise<boolean>,capability:SettingsCapabilityPort):SettingsController=>{
  let currentCategory:SettingCategory='business',currentDictionary:Dictionary|null=null,failure:SettingsFailure|null=null,busy=false,pending:Projection|null=null,categorySelection=0,dictionarySelection=0
  ...
  return {settings,dictionaries,items,get busy(){return busy},get pendingRepair(){return pending!==null},failure:()=>failure,can:visible,category:()=>currentCategory,dictionary:()=>currentDictionary,
    async selectCategory(value){const selection=++categorySelection,previous=currentCategory;currentCategory=value;try{await settings.reset()}catch(error){if(selection===categorySelection)currentCategory=previous;throw error}},
    async selectDictionary(value){const selection=++dictionarySelection,previous=currentDictionary;currentDictionary=value;try{await items.reset()}catch(error){if(selection===dictionarySelection)currentDictionary=previous;throw error}},
