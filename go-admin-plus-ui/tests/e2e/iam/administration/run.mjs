...
try {
  ...
  for (const profile of ['sqlite', 'postgres']) {
    ...
    try { const url = await waitReady(ready, host); const output = checked(chromium, ['--headless=new', '--disable-gpu', '--ignore-certificate-errors', '--no-first-run', '--no-default-browser-check', '--dump-dom', '--virtual-time-budget=45000', url], { env: environment(), stdio: 'pipe' }); if (!output.includes('IAM_ADMIN_E2E_PASS')) throw new Error(`${profile} browser scenario failed`) } finally { await stop(host) }
  }
  console.log('IAM_ADMIN_E2E_PASS')
