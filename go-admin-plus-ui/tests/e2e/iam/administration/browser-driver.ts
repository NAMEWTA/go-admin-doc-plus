...
const scenario = async () => {
  ...
  await waitUntil(() => row('browser-user')?.textContent?.includes('停用') === true, 'user disable was not rendered')
  clickRow('browser-user', 'toggle')
  await waitUntil(() => row('browser-user')?.textContent?.includes('启用') === true, 'user enable was not rendered')
