...
test('native delete uses the uniquely named product action and a test-only confirmation port', () => {
  ...
  assert.match(page, /data-testid="open-product-form"[\s\S]*>新增产品<\/button>/)
  assert.match(runner, /phase = 'product-create'[\s\S]*clickButton\(app\.child\.pid, '\u65b0\u589e\u4ea7\u54c1'\)[\s\S]*windowContains\(app\.child\.pid, '\u65b0\u589e\u4ea7\u54c1'\)/)
