...
test('accessibility query walks the native flattened element collection', () => {
  ...
  assert.match(press, /set focused of currentElement to true[\s\S]*delay 0\.2[\s\S]*set buttonPosition to position of currentElement[\s\S]*set buttonSize to size of currentElement[\s\S]*set clickPoint to \{[\s\S]*click at clickPoint/)
