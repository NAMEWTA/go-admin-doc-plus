...
test('accessibility query walks the native flattened element collection', () => {
  ...
  assert.match(press, /set scrollContainer to scrollElement[\s\S]*repeat with ancestorIndex from 1 to 12[\s\S]*role of scrollContainer is "AXScrollArea"[\s\S]*set scrollContainer to value of attribute "AXParent" of scrollContainer[\s\S]*set verticalScrollBar to value of attribute "AXVerticalScrollBar" of scrollContainer[\s\S]*set scrollMaximum to value of attribute "AXMaxValue" of verticalScrollBar[\s\S]*set value of verticalScrollBar to scrollMaximum[\s\S]*set refreshedElementsToScan to entire contents of window 1[\s\S]*repeat with refreshedCandidate in refreshedElementsToScan[\s\S]*role of refreshedCandidate is "AXButton" and name of refreshedCandidate is "产品示例" and enabled of refreshedCandidate is true[\s\S]*set refreshedElement to contents of refreshedCandidate[\s\S]*set focused of refreshedElement to true[\s\S]*delay 0\.2[\s\S]*set buttonPosition to position of refreshedElement[\s\S]*set buttonSize to size of refreshedElement[\s\S]*set clickPoint to \{[\s\S]*click at clickPoint/)
  assert.equal((press.match(/entire contents of window 1/g) ?? []).length, 2)
  assert.equal((press.match(/set value of verticalScrollBar to scrollMaximum/g) ?? []).length, 1)
  ...
  assert.doesNotMatch(press, /perform action/)
  assert.doesNotMatch(press, /click currentElement/)
  assert.doesNotMatch(press, /key code/)
