...
const waitResult = async (client, sessionId, browser, profile) => { const until = Date.now() + remaining(120_000); while (Date.now() < until) { assertChildHealthy(browser, 'Chromium'); const value = await evaluate(client, sessionId, "document.querySelector('#result')?.textContent ?? ''"); if (value.includes('SCHEDULER_E2E_PASS')) return; if (value.includes('SCHEDULER_E2E_FAIL|ASSERTION')) throw new Error(`${profile} browser assertion failed`); await delay(100) }; throw new Error(`${profile} browser timed out`) }
...
} catch (error) { failure = error instanceof Error && ['overall deadline exceeded', 'browser fixture build failed', 'HTTPS host compile check failed', 'test child cleanup failed'].includes(error.message) ? error.message : 'scheduler E2E execution failed' }
