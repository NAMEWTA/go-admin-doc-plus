...
if (!chromium || !postgres) { console.error('ORGANIZATION_E2E_RUN_FAIL|required environment is missing'); process.exit(1) }

const root = dirname(fileURLToPath(import.meta.url))
const uiRoot = resolve(root, '../../..')
const backend = resolve(uiRoot, '../go-admin-plus')
const temporary = mkdtempSync(join(tmpdir(), 'go-admin-organization-e2e-'))
...
const waitForResult = async (client, sessionId, browser, profile) => {
  const deadline = Date.now() + remaining(120_000)
  while (Date.now() < deadline) {
...
const runProfile = async (profile) => {
  ...
  const host = spawnTracked(spawn, 'go', ['test', './test/organization', '-run', '^TestOrganizationBrowserHarnessServer$', '-count=1', '-v'], {
    cwd: backend,
    env: environment({ GO_ADMIN_ORGANIZATION_E2E_SERVE: '1', GO_ADMIN_ORGANIZATION_E2E_PROFILE: profile, GO_ADMIN_ORGANIZATION_E2E_READY_FILE: ready, GO_ADMIN_ORGANIZATION_E2E_STATIC_DIR: staticRoot }, profile === 'postgres'),
...
let failure = ''
try {
  checked('pnpm', ['exec', 'vite', 'build', '--config', join(root, 'vite.config.ts')], { cwd: uiRoot, env: environment({ GO_ADMIN_ORGANIZATION_E2E_OUT_DIR: staticRoot }), stdio: 'pipe', failure: 'browser fixture build failed' })
  checked('go', ['test', './test/organization', '-run', '^TestOrganizationBrowserHarnessServer$', '-count=1'], { cwd: backend, env: environment({ GO_ADMIN_ORGANIZATION_E2E_SERVE: '0' }), stdio: 'pipe', failure: 'HTTPS host compile self-check failed' })
...
if (failure) { console.error(`ORGANIZATION_E2E_RUN_FAIL|${failure}`); process.exitCode = 1 }
else console.log('ORGANIZATION_E2E_PASS profiles=sqlite,postgres')
