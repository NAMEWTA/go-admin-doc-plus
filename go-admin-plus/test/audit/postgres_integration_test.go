...
func TestAuditPostgresMigrationProjectionQueryAndCleanup(t *testing.T) {
	...
	enqueue(t, db, store, outbox.Event{ID: "audit-pg-event-001", Topic: audit.TopicOperationUpdated, BusinessKey: "resource:demo:record-pg:revision-2", Payload: []byte(`{"source":"server","actorType":"system"}`), OccurredAt: now.Add(-60 * 24 * time.Hour)})
	dispatch(t, db, store, mustConsumers(t), now)
	if _, err := db.Bun().ExecContext(ctx, "UPDATE audit_facts SET payload = ? WHERE event_id = ?", []byte(`{"source":"server","actorType":"system"}`), "audit-pg-event-001"); err != nil {
	...
	if err != nil || page.Total != 1 || page.Records[0].Subject != "demo:record-pg" {
