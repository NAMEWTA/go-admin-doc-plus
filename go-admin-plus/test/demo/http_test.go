...
func (requestAuthenticator) AuthorizeRequest(_ context.Context, token, csrf string, mutation bool) (demo.RequestIdentity, error) {
	...
	return demo.RequestIdentity{ActorID: "http-actor", CSRF: strings.Repeat("c", 32)}, nil
}
...
func TestHTTPContractCSRFPermissionAndStableProblems(t *testing.T) {
	...
	handler, err := demo.NewHTTPHandler(service, requestAuthenticator{}, "__Host-test", func(*http.Request) string { return "0123456789abcdef" })
	if err != nil {
		t.Fatal(err)
	}
	...
	if response.Code != 201 || response.Header().Get("X-CSRF-Token") != strings.Repeat("c", 32) {
	...
	deniedHandler, err := demo.NewHTTPHandler(denied, requestAuthenticator{}, "__Host-test", func(*http.Request) string { return "0123456789abcdef" })
	if err != nil {
		t.Fatal(err)
	}
