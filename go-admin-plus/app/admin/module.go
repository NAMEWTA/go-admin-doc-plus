...
func (*Module) ID() string { return "admin" }

func (*Module) RegisterRoutes(handler http.Handler) error {
	engine, ok := handler.(*gin.Engine)
	if !ok {
		return fmt.Errorf("admin module requires *gin.Engine, got %T", handler)
	}
	...
	router.InitSysRouter(engine, authMiddleware)
	router.InitExamplesRouter(engine, authMiddleware)
	return nil
}
