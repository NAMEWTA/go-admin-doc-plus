...
- **E2E disposition：** deferred：Windows 原生安装、运行、重启和卸载场景保留到全部 Ticket 实现集成后的统一系统 E2E；正式签名和凭据使用仍需独立授权。
- **E2E owner/environment：** Lead / 最终系统候选的受保护 Windows 环境；逐 Ticket source-worktree 与 parent-candidate 不运行或声明 E2E 通过。
