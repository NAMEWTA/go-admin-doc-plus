...
G7 RED 盘点证明旧目录不是全部孤立文件：未被正式入口消费的 expand 期 `internal/application` 根包、`profile`、旧 Desktop host、root platform adapter、legacy command/template/test 仍引用待删除的 `app/common/api/tenant`；根 Task、CI、Windows tracer 与聚合 release manifest 也仍调用旧 root main、`sqlite3` tag、Wails、旧前端名和 unsigned-self-use。若只按初始路径表删除 12 项治理文件与显式旧目录，全量构建立即失败，且 AC-002/AC-029/AC-033 会形成伪通过。
