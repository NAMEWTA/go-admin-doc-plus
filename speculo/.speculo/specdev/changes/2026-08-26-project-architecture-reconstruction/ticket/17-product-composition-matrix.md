...
- **批准偏差：** `T17-D01` 开放 app-shell product export/checks；`T17-D02` 只删除 Generator 双方言迁移 `Down` 段；stage-1 checkpoint `92a0ff8` 后，`T17-D03` 精确开放产品 UI、双 App、Browser parser、Desktop adapter/Rust/sidecar 与必需的 manifest/Vitest/boundary/lock importer。各 domain/web-domain 业务实现、模块生成合同、无关根脚本/importer 继续只读。
