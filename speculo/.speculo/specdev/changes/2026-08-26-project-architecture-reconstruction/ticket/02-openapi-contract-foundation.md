...
- `T02-D01`：原路径合同无法实现已批准的 `task contract:lint generate:check` 和固定官方生成器。Lead/Ticket owner 于 2026-08-26 批准最小扩展：T-02 只新增这两个根任务，只写入 oapi-codegen/生成 transport 所需的 Go module 记录、合同工具/客户端所需的 pnpm lock，并仅为实际 registry 当前版 `@redocly/cli@2.48.0` 增加精确 release-age allowlist；不改变 Spec、ADR、产品 API、安全策略或后续 owner 的其他职责。
