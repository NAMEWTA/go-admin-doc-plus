...
|---|---|---|---|---|---|---|---|---|---|---|
| T-01 | `<Path>{roots.state}/specdev/changes/2026-08-26-project-architecture-reconstruction/ticket/01-root-governance-command-plane.md</Path>` | 根治理与统一任务入口 | — | deep | high | yes | codex-root | AC-001, AC-002 | W0 / G0 | in_progress |
...
| 共享路径 | 唯一 owner | 消费方式 |
|---|---|---|
| `<Path>Taskfile.yml</Path>`、`<Path>.husky/**</Path>`、根分类脚本 | T-01 | 后续 Ticket 只读调用 |
...
| Database/migration API、`<Path>go-admin-plus/go.mod</Path>`、`<Path>go-admin-plus/go.sum</Path>` | T-04 | 模块实现自有 Provider，不改依赖清单 |
| Workspace/lock/app-shell core/platform/ui | T-05 | 模块只写预建包的 `src` |
