...
- 最终文档审计发现两份项目 Agent Skills 仍描述已删除架构，且根零兼容与文档检查未扫描 `.agents/skills`。`T21-D04` 以实现提交 `95663eb35d8554f5f66c9b8fdc965e9914943d50`（tree `23427ee92cec831ae1371fcc646fa30dcfc8f460`）重写业务模块/列表页工作流并扩展治理覆盖；后置 `2906c2c459834fc7b6d5a9b9a5250960875f9811` 删除残留 UI 框架耦合描述。两份 Skill 校验和临时打包、compatibility/docs/architecture、质量回归与 diff 检查全绿，产品源码零变更且未运行 E2E。
...
- 后续 UI/CSS 精确历史对照证明上述源码卫生审计未覆盖最终 cascade 保真：重构前 `sidebar.scss` 的菜单条目为 `50px`、`TagsView.vue` 的可见页签带为 `34px`，而当前 Shell 为 `40/42px` 与 `40px`；Administration/Audit/Organization/Scheduler/Account 等 scoped CSS 还会覆盖共享 `20px/34px` 紧凑管理合同。`T05-D07` 以 implementation/result `a38c7cf3b4f4e756393f487a6cc0f45a2e0449e2`（tree `f510a297cb412c9c9b8b29f828f3dfbe63493414`）恢复精确尺寸、补齐适用 token、移除九个领域页的 base surface/control/table 覆盖，并新增逐页防回退合同；lint、全 typecheck、九 driver 纯编译、28 files/144 tests、7 boundaries、双 build 和全部根静态 Gate 通过，未运行 E2E。
- 第五轮登录/账户基线审计继续发现历史已有的密码显隐/焦点归还、终端逐行动效/`620px/10px`/`1100px` 断点，以及个人中心 `1:3` 身份摘要/资料密码 tabs/键盘切换/确认密码在重构后丢失；这些均属于当前 T-06/`AC-012` 仍保留的页面与密码能力。`T05-D08` 以 implementation chain `0db2125a06b470a0196db53fd7a09d060c11acec` -> result `9497242d44347dba5a1a84cc12ccb13c1beb3c41`（tree `083a483135c4525ffcfc6ffa78e4db9075cea4dd`）恢复上述结构和交互，只保留 username/displayName/email/avatar metadata，增加确认密码 Controller 前阻断与头像清空归一化；验证码、旧资料字段、旧头像上传和兼容链零恢复。lint、全 typecheck、九 driver 纯编译、29 files/146 tests、7 boundaries、双 build 和全部根静态 Gate 通过，未运行 E2E。
- 第六轮保留管理页审计继续发现英文危险确认、Settings 保存错误经过删除确认、Audit 详情游离于共享 modal/focus 合同、Settings 三个列表与 Generator 数据表缺少分页入口，以及 Generator 预览文件选择跨重新开始/新预览残留。`T05-D09` 以 `main@73ea23c6a0647470ef098dafbb669327262c28c2` 为 base，implementation/result `52099ea78f435829f52f73b9058e736fa97a6bd3`（tree `a70dd5ad598f7f6fdc479d7656c3a6a11ea90cec`）完成中文且领域精确的危险确认、Settings 仅删除确认、共享 Audit modal/focus 恢复、四个真实分页入口和 Generator 选择生命周期重置；API、Permission、删减能力和兼容路径零变更。lint、全 typecheck、九 driver 纯编译、29 files/149 tests、7 boundaries、双 build、固定 Task `v3.48.0` 根合同与 2 项质量回归通过，未运行 E2E。
- 第七轮 `AC-035`/源码合同审计继续发现五个 retained list 隐藏 page-size、Files/Demo 未完整暴露已有排序、Files 的创建时间合同与展示字段漂移、transport 时间/任务 key 直出，以及 Settings/Generator/Demo 未完整保留失败输入或投影已有原生约束。`T05-D10` 以 `main@5a8d4baec9e3b913fa3d9f2a30c94c280565b8a7` 为 base，implementation/result `a29ae4c84429065c6f28b42148e362a188725baf`（tree `bd1d54564e4e82a44cc0398a72001b78a266ea9f`）补齐完整分页、双向排序、合同一致的 Files 时间、本地化时间/任务标签、失败保留输入、busy lock 与原生字段约束；API、controller、Permission、删减能力和兼容路径零变更。全 workspace lint/typecheck、九 driver 纯编译、29 files/149 tests、7 boundaries、双 build、固定 Task `v3.48.0` 根合同与 2 项质量回归通过，未运行 E2E。
- 最终仓库级零兼容审计发现历史 `module go-admin` 与 151 个 `go-admin/internal/...` import 仍未迁移，且 `compatibility-zero` 不扫描 `.mod`，与 CONTEXT/ADR 的旧 module path 零命中要求直接冲突。`T21-D07` 以 `main@4968ca5a8e2c715fe2b13c9efab7b3e85e276e32` 为 base，implementation/result `200829ae39ba2f0aa9e101399467c86db02ff57f`（tree `1056edcd6b8cc5178f06bde44120031419fe478e`）原子迁移到 `github.com/NAMEWTA/go-admin-plus/go-admin-plus`，同步 Generator 模板并加固 architecture/compatibility 负向门禁；无 replace、别名或兼容路径。Go normal/SQLite/race、三轮 Generator、vet/build/CGO0/module、OpenAPI 28/28/API 7/7/generate-check、前端 29 files/149 tests/7 boundaries/双 build、固定 Task 根合同、production sidecar、Tauri 2 Rust 20/21 tests/clippy/release build 和 release policy 16/16 全绿，未运行 E2E。
- 最终跨语言命名审计继续发现 24 个 pnpm 内部 package、workspace 依赖/import/lock、Generator/OpenAPI 以及 Task/CI/发行 filter 仍使用历史 scope，与根 `@go-admin-plus/workspace` 和无兼容重命名冲突。`T21-D08` 以 `main@dd1f6ba1e227a039ccaba3364278ae94b6223cfd` 为 base，implementation/result `a342c99e1f38db504de4909c3a69c7913dd31374`（tree `ebc6c5d1349eca682fc9e174bccd94baacb94612`）原子迁移到 `@go-admin-plus/*`，加固 workspace/architecture/compatibility 门禁，并让前端生成 wrapper 委托唯一根 OpenAPI 生成器；无 alias、双 scope、UI/CSS 或业务变化。25-project frozen install、前端 lint/typecheck/29 files 149 tests/7 boundaries/双 build、合同 28/28/API 7/7/generate-check、Go normal/SQLite 与 Generator、双 vet/build/module verify、固定 Task 根门禁/全目标构建、release policy 16/16、production sidecar及 Tauri 2 Rust 20/21 tests/clippy/release build 全绿，未运行 E2E。
- 最终非 E2E 完成审计实际执行三个根 package target 后，确认 Web 从临时输出构建却归档不存在的根 `dist`，Desktop 漏传 `custom-protocol` 并在通用本地命令进入签名前 DMG。`T21-D09` 以 `main@82d7831201779bc8ee82c7d19cdd59eaea4b37cb` 为 base，implementation/result `15e97506c982a06dbc8454923762f5b42bd3bb99`（tree `578035dd681947e0915e10e1feeb093910cb6cfa`）改为 artifacts staging 原子 Web tar，以及按 `GOHOSTOS` 选择 macOS App/Windows NSIS 并固定 production protocol；受保护 DMG/签名/公证职责不变。含空格路径 Server/Web package、macOS App 主程序/sidecar/plist/marker、前端 lint/typecheck/29 files 149 tests/8 boundaries、Task/release/static Gate 与 Cargo metadata 全绿，未运行 E2E。
- 最终命令与文档审计确认 Web 指南承诺连接本地 `127.0.0.1:8080`、客户端统一使用相对 `/api`，但 Vite dev server 没有代理，且根 Task/CI/scripts/release/deploy 没有拒绝不存在 workspace package filter 的回归。`T21-D10` 以 `main@95d45bad3dd276a90605641160a0c47a3833f1b4` 为 base，implementation/result `f5d55ba47190604f836d84e3a19528ceb8c21346`（tree `6010068b3c082e42cde894cc4e1a6f9d62f762c8`）增加无 rewrite 的 loopback `/api` proxy、真实 Vite config 与 25-package 命令面检查，并同步 build/package/受保护发行文档。frontend lint、全 typecheck、29 files/149 tests、10 boundaries、双 build、真实 Web package、根静态/Task Gate 与 release policy `16/16` 全绿；UI/CSS 和业务零变化，用户 `5173` 进程保持未动且未运行 E2E。
- 最终当前态配置审计发现 SpecDev runtime verification 仍调用已删除前端目录、旧 Make/私有构建入口；真实执行迁移后的根测试又证明 D09/D10 的根 package/command 断言错误进入 Generator 独立 workspace。`T21-D11` 以 `main@7d078a528c403e6574822e932ca68f7eecc07f57` 为 base，implementation/result `6e55fd64c3a8acd8c91e6d342da46ab1dc0b372d`（tree `def28ab80f802f3cc27dcbf7d2eeff0f19ceadd5`）把四条 runtime verification 固定到根 Task/当前 pnpm workspace，把 25-package/本地 packaging 合同迁至根 architecture gate，并保留 8 项自包含 workspace 边界。首次 Generator `190.97s` 有效失败；修正后 normal/SQLite Generator `351.677s`/`364.702s`、双方言 Go、29 files/149 frontend tests、workspace `8/8`、typecheck/lint/全目标 build、architecture `6/6`、quality `7/7` 与 compatibility-zero 全绿；UI/CSS、业务和 E2E 零变化。
- 最终验证覆盖审计证明根 typecheck 漏跑 3 个已声明检查的 package 和 6/10 test projects，中央 Vitest 手工清单漏跑 8 个 unit spec，`task test` 也未执行 29 项 harness Node 单元测试。`T21-D12` 以 `main@f794a3e757049ae73bdb4982d7dec6cf98758c95` 为 base，implementation/result `11e358f0381612a327337ba94b86c30572b81c04`（tree `4d2a04376df5ad794744cb2cf210735ad3df5320`）改为递归 package typecheck、完整 test-project 编译、集合式 Vitest 发现及 Generator 可复制的 Node test runner，并由根 architecture gate 拒绝覆盖回退。normal/SQLite Generator `377.530s`/`365.516s`、双方言 Go、37 files/191 Vitest、37 Node tests、完整 typecheck、lint、全目标 build、architecture `11/11`、quality `12/12`、release policy `16/16`、OpenAPI lint/generate-check 与八模块集合全绿；UI/CSS、产品、合同、schema、发行行为和 E2E 零变化。
- 最终构建完成性审计证明 `task build TARGET=desktop` 仍只编译 Vite WebView，`TARGET=all` 也遗漏 host sidecar 与 Rust/Tauri executable，与“Server/sidecar/Web/Desktop builds”证据冲突。`T21-D13` 以 `main@4ae448301b08e0593ea85988f8903b1141b78fc2` 为 base，implementation/result `8d85159badcd5d4531bc6f17d4a29783fa2c24e4`（tree `2b3f3f1e2978311baac1e97c881bc30cec9dae48`）让 `desktop`/`all` 先构建当前宿主 sidecar，再执行 `custom-protocol --no-bundle` Tauri release build；package/signing/notarization 边界不变。固定 Task `v3.48.0` 的真实全目标构建生成 arm64 Server、sidecar 与 Tauri host，Web production build通过且无新 bundle/test-control marker；lint/全 typecheck、architecture `12/12`、quality `13/13`、五项根治理与 release policy `16/16` 全绿，UI/CSS、产品和 E2E 零变化。
- 本地原生构建闭合后继续审计 CI，确认 macOS `desktop-rust` job 虽构建 sidecar，却未安装 pnpm workspace，并以 `cargo check` 结束，因此不会运行 Tauri `beforeBuildCommand` 或链接 release Desktop executable。`T21-D14` 以 `main@8e5a8ad3beab5c28258d31c21cda08823b6930e0` 为 base，implementation/result `2ac8e3fa1020caa38384d8888b2d85fa21f07ff4`（tree `4d383fb94bfb5d3919d1953b4cd9d09f23155317`）让 CI 固定 pnpm `11.1.3`、frozen install，保留 sidecar、Rust tests/strict clippy，并执行 `custom-protocol --no-bundle` Tauri build；架构门禁按 Desktop job 局部拒绝三项回退。等价本机构建通过 20 个 Rust tests、Clippy、WebView production build 和 `45.49s` release link；lint、quality `14/14`、五项根治理、release policy `16/16`、YAML/shell/diff 全绿，未 bundle、签名、发布或运行 E2E。
- 本地命令面复核随后证明此前通过固定 Node 实体路径验证会掩盖真实 Volta 环境问题：Volta Node runner 从 shell 子进程 PATH 移除同目录 pnpm shim，根 lint/test/build/package 无法直接使用已安装的 `11.1.3`；直接 `corepack pnpm` 又不能供嵌套 package script 解析自身。`T21-D15` 以 `main@303565aa4ccc3d0fa0a48b38147351ab1cf9cc76` 为 base，implementation/result `7e21956d33567cc49e148b0789c9781d5d556843`（tree `7bebf7898cfbf4ef3b5531f2da82f12cbf645e2f`）改为优先直接 pnpm、缺失时在根忽略 artifacts 中生成并导出标准 Corepack shim，六个前端脚本统一经 resolver 调用并保持 exec 退出码。原始 Volta/Task `v3.48.0` 路径下 lint、Task contract `5/5`、frontend `37 files/191 + 37 Node`、全目标 Server/Web/sidecar/Tauri build（release link `1m06s`）和 Web tar package 全绿；合并质量 `20/20`、五项根治理与 release policy `16/16` 通过，UI/CSS、产品行为和 E2E 零变化。
- D15 后的真实完整 `task test` 暴露后端 Generator 仍无法从 Volta Node 子进程发现 pnpm，且仓库未完整声明 Task、Node CI 与 Rust 最低/CI 基线。`T21-D16/D17` 以 `main@b97f2e20f30b0690bdcd98f69aa77e87207e365e` 为 base，implementation/result `e62519958ed83bf42bd0bc76cd3848c0023ff9c0`（tree `ae7a83851a3682757ae2790ab28324cc73bb274b`）固定 Task `3.48.0` 和完整工具链文档/门禁，并让前后端、合同 wrapper 与 backend CI 共用精确 `pnpm@11.1.3` resolver。原始 Volta PATH 下双方言全量测试（Generator `577.567s`/`406.933s`）、37 files/191 Vitest、37 Node、lint、合同 28/28/API 7/7、quality 17/17、release policy 16/16 与 Server/Web/sidecar/Tauri 全目标 build（release link `46.56s`）全绿；UI/CSS、产品行为和 E2E 零变化。
- 最终 AC-011/AC-022 审计发现 Web runtime 丢失 IAM `dataScope` 且 Shell 对缺失范围默认 `all`，完整并发 Go Gate 又稳定复现 Volta Node shim 的 OS error 35。`T21-D18` 以 `main@8ca17e4d8b8d32ffa266540ebbaa20a70ee4b743` 为 base，implementation/result `09a3619774294f6178850d52e3054a5b5f42dbe8`（tree `618947a990d1b0a4f09b9c8322fc90a230467c00`）让 Web/Desktop 共用必填 `self|all` 身份合同、删除 Shell 默认放大，并让 Generator 解析 Volta 真实 Node runtime。frontend 37 files/192 Vitest + 37 Node、normal/SQLite Generator `366.924s`/`364.831s`、双方言 Go、完整 lint/typecheck、Server/Web/sidecar/Tauri build、Task 3.48.0 根合同、OpenAPI 28/28/API 7/7/generate-check、quality 17/17 与 release policy 16/16 全绿；UI/CSS、公共模块 OpenAPI、schema、迁移、依赖锁和 E2E 零变化。
- 最终 Desktop production artifact 审计发现 Vite 虽摇除环境分支 JavaScript，却仍把 `.native-e2e` scoped CSS 写入 production bundle，既有 restore check 只搜索一项按钮文本而无法证明测试控制代码零残留。`T21-D19` 以 `main@4ba72eadd7ef118df3c126d6ae5b7bbf8330fd8f` 为 base，implementation/result `9002a383c669ec591d2cfc384924addaed2b6821`（tree `53cea92dcce8f9d1940cc031b23efda4a30273ca`）把测试 UI/CSS 移入仅由显式 `native-e2e` mode 选择的独立入口，并让每次 production build 逐字节拒绝 test-control route/marker。前端 lint/全 typecheck、37 files/192 Vitest、39 Node、Server/Web/sidecar/Tauri 全目标 build、Task 3.48.0 五项根门禁、OpenAPI 28/28/API 7/7/generate-check、quality 17/17 与 release policy 16/16 全绿；产品可见 UI/CSS 和运行时行为不变，未运行 E2E。
- D19 production boundary 的完备性审计继续发现 WebView 只列出五项 marker，而 sidecar/host restore 仍只扫描单一路由。`T21-D20` 以 `main@2661de5c1e020f416d02e5b67e81fb1beed8c42c` 为 base，implementation/result `9cb4002532ff869eed01dc3f57fb23477a4574be`（tree `7eb7361f97aa5caa792bd27a4a460c97d5f2a13c`）统一 route/mode/env/feature/全部 E2E 文案与 fixture marker，并让 WebView 树、sidecar、host 共用同一 verifier。定向 `14/14`、frontend 37 files/192 Vitest + 39 Node、完整 lint/typecheck、production Vite/sidecar/Tauri build（link `47.71s`）、重建后三制品扫描、quality `17/17`、release policy `16/16` 与治理/架构/零兼容/文档门禁全绿；产品、UI/CSS、合同、数据库和 E2E 零变化。
- D20 二进制扫描接线审计继续证明 sidecar/host 只在暂停的 native restore 中受检，公开 build/package、Desktop CI 与两平台正式发行可以绕过；短通配 marker 还可能误撞二进制。`T21-D21` 以 `main@70afa0660968b5ec642057ac2a1ea1c36ac7b37c` 为 base，implementation/result `15a54f45398c88261767e62ee476ecab17bf90b3`（tree `22bedf78d64775989ab364526f6a717fb1de693b`）增加复用既有 target-triple 合同的跨平台 build verifier，把最终 WebView/sidecar/host 扫描接入本地 build/package、CI、macOS Universal 与 Windows NSIS，并将 marker 收紧为精确长字符串。真实 production Desktop build 链接 `46.92s` 后主动报告 `DESKTOP_PRODUCTION_BUILD_PASS`；frontend 37 files/192 Vitest + 40 Node、完整 lint/typecheck、quality 17/17、release policy 16/16、治理/架构/零兼容/文档与语法门禁全绿。当前 Mac 无 `pwsh`，Windows 仅记录静态/policy/CRLF 证据；未运行 E2E、签名、安装或发布。
- 最终 Spec-to-source adapter 审计证明 `PlatformPort` 仍无实现/消费，Desktop Files 仍使用浏览器 DOM 文件接缝，未满足双 adapter 与 Desktop 文件/通知/剪贴板合同。`T21-D22` 的审计 base 为 `5bac141948c118ed08283656bca555bc38d09fe8`，授权 checkpoint 为 `9be09eccc38d26a5f62dd75629c88e2a8fbc689d`，implementation/result `aa502221356f7bbd9c3d7619ea10272f4a5971cf`（tree `bb167a772d714b427f1503e6cf69a579a1097bb5`）让 Web/Desktop App 显式注入各自 Platform adapter，Desktop 文件选择/保存经四个精确 allowlist Rust command 且不暴露路径或通用 filesystem 权限，剪贴板仅写、通知仅发送；Web 文件输入与现有 CSS 保持不变。frontend 37 files/195 Vitest + 41 Node、完整 lint/typecheck、Rust 23/23、strict clippy/fmt、Server/Web/sidecar/Tauri production 全目标构建、quality 17/17、release policy 16/16 与四项根静态门禁全绿；未运行 E2E、签名、安装或发布。
- D22 后的真实二进制数据路径审计证明 Go Files 成功下载固定输出 `application/octet-stream`，而 Desktop Rust proxy 只接受 PNG/JPEG/PDF/TXT，导致所有 Desktop 下载确定性失败。`T21-D23` 以 `main@b9573ac16011073c8f33ca38bb700827523bed85` 为审计 base，授权 checkpoint `63bb941e904a0c14a659f1af2f4c3afbde91bcc0`，implementation/result `0554d320b74c3e3be7f2c8db8c52e38af81b42b1`（tree `108d70c73529f18f9bbb116aa526aed43a331e59`）把 Rust IPC 与 Desktop fetch bridge 收敛到唯一 exact-key、canonical、10 MiB 有界 octet-stream envelope，错误响应保持 JSON；不增加 MIME 兼容分支。Rust 24/24、frontend 37 files/196 Vitest + 41 Node、完整 lint/typecheck、Generator 精确失败重试 `345.659s`、OpenAPI 28/28/API 7/7、quality 17/17、release policy 16/16、根静态门禁和 Server/Web/SQLite sidecar/Tauri production 全目标构建全绿；Files API/OpenAPI、数据库、页面、UI/CSS 和受保护发行零变化，未运行 E2E、签名、安装或发布。
- 最终 Desktop source graph 审计继续确认 T-17 切换到全产品 `product_contract.rs` 后，525 行 `demo_contract.rs` 仍作为未声明 module 留存，其 validator/tests 从不编译，并独占 `time`/`uuid` 两个直接 dependency edges。`T21-D24` 以 `main@cf864c28df223a7813ce1afba5b5b0d27e9bc47f` 为审计 base，授权 checkpoint `5f9878fa5de308d36f9118630c959a4ecd820ca4`，implementation/result `a1f61f66b05a6beb4f68455f03f952909dfda269`（tree `cf38d7be04ec335d6f87936592ad6ef927805558`）删除死源码和无用直接依赖，并让 compatibility-zero 精确拒绝旧路径恢复。Rust 24/24、quality 18/18、release policy 16/16、根静态门禁和 Server/Web/SQLite sidecar/Tauri production 全目标构建全绿；运行行为、API、数据库、页面、UI/CSS 与受保护发行零变化，未运行 E2E、签名、安装或发布。
- 最终前端 package/source graph 审计继续确认共享 app-shell 直接依赖 Browser adapter，导致 Desktop 经共享产品组合间接消费 Browser runtime；Browser adapter 又仅为 Files client 反向依赖 Files Domain。`T21-D25` 以 `main@babbf1f723e7ad41d319cc82729272a734c0c86f` 为审计 base，授权 checkpoint `763f7c78f278737f4d6350c0ba96aba7a5e00579`，implementation/result `d236c21368f0126bc6ec3a36ef03aec3e619969e`（tree `9b61f312f159574e4553d83af694274d38c312e7`）把原行为 Files client/test 迁入 Files Web Domain，删除两条反向 workspace edge，并以标准 Browser package checks 和 boundary gate 防止回归。frontend 37 files/196 Vitest + 41 Node、完整 lint/typecheck、quality 18/18、release policy 16/16、根静态门禁和 Server/Web/SQLite sidecar/Tauri 2 production 全目标构建全绿；API、数据库、页面 template/style、UI/CSS 与受保护发行零变化，未运行 E2E、签名、安装或发布。
- D25 后继续审计 Desktop adapter graph，确认未消费 `createDesktopDemoClient` 单独保留 adapter→Demo Domain 依赖，而生产与 native-e2e 产品组合已统一使用 `createDesktopFetch` 加 Web Domain client。`T21-D26` 以 `main@c172320a846efe71cf6a040818e730413a6abaf9` 为审计 base，授权 checkpoint `8abc93446b6aefebce2f74be54a811b42ac56238`，implementation/result `124dd32344f000fab655783782d6e54c5f4fbecb`（tree `3f3cc2a823966f9cf2402e87e6d2c04f91e8132b`）删除死 client/status mapper/自测和依赖边，保留通用 transport、Session/Stronghold、Platform 与 native control，并新增 Desktop adapter dependency allowlist。frontend 37 files/195 Vitest + 42 Node、完整 lint/typecheck、quality 18/18、release policy 16/16、根静态门禁和 Server/Web/SQLite sidecar/Tauri 2 production 全目标构建全绿；产品/API/数据库/UI/CSS 与受保护发行零变化，未运行 E2E、签名、安装或发布。
- 最终 25-workspace 集合审计证明 Desktop adapter、Platform 与 UI 缺少 package-local typecheck，Desktop adapter 有 spec 却无 test；递归 `--if-present` 和间接消费者不能证明 package 独立完整。`T21-D27` 以 `main@29621cf01fb7b3b9697c6443dab2b680c3f2c7d2` 为审计 base，授权 checkpoint `c16a2e3b90cde3b21ad592763801c90abb0d9048`，implementation/result `b20e56c771d08d47da25c68dde251d1a38c07ae5`（tree `c1ae58477ebc0065f8c5577a55f536e3eb8d13a8`）补齐三个 strict package contract，并让 architecture gate 动态要求 24/24 package typecheck 与 spec-owning package test。frontend 37 files/195 Vitest + 42 Node、完整 24 package/十 test-project typecheck、quality 20/20、release policy 16/16、根静态门禁和 Server/Web/SQLite sidecar/Tauri 2 production 全目标构建全绿；产品源码、API、数据库、UI/CSS 与受保护发行零变化，未运行 E2E、签名、安装或发布。
- 最终 Go production import graph 审计证明既有 architecture gate 没有实现 ADR-006/ADR-012 的导入图验证，七个非 IAM 业务模块仍直接依赖 IAM 具体包，Generator 还会复制该模式。`T21-D28` 以 `main@f76649efa829a209b91bb9729ce5c90d4dec0371` 为审计 base，授权 checkpoint `cb3a428d3805723d58f3fe40022d6aaa78fbf516` 与 D28-A01 checkpoint `fe4345ff00b6065426b7473cc85eb962c3dc057c` 建立 Go AST 跨模块失败门禁，把共享 capability 值收敛到 contracts，并将 Session、Authorization、Organization Projection 与 Login Fact adapter 全部移到 app-owned adapter 集合，由正式 product composition root 显式注入；受跟踪测试组合根复用同一类型化 adapter，禁止复制或恢复模块内 adapter。implementation/result `c648a32ad5e20efe2059fe7993af3fa751ce5887`（tree `bb8320f5179ea9c4a04d7762986ac06f961cffa9`）已通过普通/SQLite 全量 Go、race、vet、真实 Generator 隔离编译、根架构/零兼容扫描及 Server/Web/Tauri 2 production 全目标构建；消费者保留最小 port，旧 `NewIAM*`、隐式 IAM 默认构造与跨顶层 production module edge 均为零。完整证据见 T-21 Evidence 第 29 节，E2E 暂停状态不变。
- D28 后按 ADR-012/ADR-016 继续审计模块数据库所有权，当前生产 SQL 人工清单虽未发现跨模块表访问，但仓库没有从 migration 推导 table owner 的自动化，无法阻止未来模块直接查询其他模块表。`T21-D29` 以 `main@5e76dee3a8c1f600d0b32fc865ed11a896b61b06` 为审计 base、授权 checkpoint `cba2cec93eaab9a4ae5e7c8bd1c827283d4c784f`，从双方言 migration 推导唯一 owner，并以 SQL table-position 词法扫描和 own/foreign/test/conflict fixture 固定生产边界。implementation/result `e6c300c1fdd891602ddf2b4303596b1917989e4e`（tree `f9c8d3cab05d0a440db1a47a7f47643408e7d254`）已通过普通/SQLite 全量 Go、race、vet、根架构/零兼容与 Server/Web/Tauri 2 production 全目标构建；产品、schema/migration、API、前端、UI/CSS、发行与暂停 E2E 不变。完整证据见 T-21 Evidence 第 30 节。
- D29 GREEN 后反查 Generator 发布前接缝，确认 compile gate 只测试/构建目标模块而不运行 repository application architecture package，不能证明生成物实际通过 import/table owner 门禁。`T21-D29-A01` 以 `main@cba2cec93eaab9a4ae5e7c8bd1c827283d4c784f` 为审计 base、授权 checkpoint `15c58d8cc9293190bae819a6d4f13b22f832b257`，把命令清单变为可测试合同并在目标模块 test/build 前运行 application architecture tests；同一 `e6c300c1fdd891602ddf2b4303596b1917989e4e` result 的真实 canonical Generator 隔离链与双方言全量矩阵均通过。生成内容、自动产品注册 OUT 边界、API、schema/migration、前端、UI/CSS、发行和暂停 E2E 不变。
- D29 后按 AC-026 与 ADR-006 审计正式运维路由和后端层级，确认 Server 只接入三项 endpoint，而 metrics/status 仅存在于零生产引用且反向依赖 app/kernel 的 platform/observability；Desktop 也未复用 product readiness。`T21-D30` 以 `main@638e2a0a43cbc4fc8994ddbce9b6d37eaffe83c4` 为审计 base、授权 checkpoint `de98a1f58f532812d0f4bc997eb16d04ccf9d15f`，统一 application health 五端点和三 profile、接入 Server/受 control 保护的 Desktop、删除重复 observability、把 host lifecycle 从 app 迁至 host，并建立低层 import/path 失败门禁。implementation/result `6c01fd983e2f26b4fb01c71be8ffcd7a11fdcb81`（tree `364840919749cb3ab736a7e18c05b6c63486ae7f`）已通过普通/SQLite 全量 Go、race、vet、真实 Generator、根静态门禁与 Server/Web/Tauri 2 全目标 production build；完整证据见 T-21 Evidence 第 31 节，UI/CSS 和暂停 E2E 不变。
