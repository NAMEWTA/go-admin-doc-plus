...
- 最终 Go production import graph 审计证明既有 architecture gate 没有实现 ADR-006/ADR-012 的导入图验证，七个非 IAM 业务模块仍直接依赖 IAM 具体包，Generator 还会复制该模式。`T21-D28` 以 `main@f76649efa829a209b91bb9729ce5c90d4dec0371` 为审计 base，授权用 Go AST 建立跨模块失败门禁，把共享 capability 值收敛到 contracts，并将 Session、Authorization、Organization Projection 与 Login Fact adapter 全部移到 app-owned adapter 集合，由正式 product composition root 显式注入；`D28-A01` 允许受跟踪测试组合根复用同一类型化 adapter，禁止复制或恢复模块内 adapter。消费者保留最小 port，旧 `NewIAM*` 与隐式 IAM 默认构造零兼容删除。实现、非 E2E 验证与 result SHA 待 Evidence 回写。
