...
- D28 后按 ADR-012/ADR-016 继续审计模块数据库所有权，当前生产 SQL 人工清单虽未发现跨模块表访问，但仓库没有从 migration 推导表 owner 的自动化，无法阻止未来模块直接查询其他模块表。`T21-D29` 以 `main@5e76dee3a8c1f600d0b32fc865ed11a896b61b06` 为审计 base，授权只在 Go architecture test 中建立双方言 migration 表所有权和非测试生产 SQL 字面量门禁，并用 own/foreign/test fixture 证明拒绝能力；产品、schema/migration、API、前端、UI/CSS、发行与暂停 E2E 不变。实现、非 E2E 验证与 result SHA 待 Evidence 回写。
- D29 GREEN 后反查 Generator 发布前接缝，确认 compile gate 只测试/构建目标模块而不运行 repository application architecture package，不能证明生成物实际通过 import/table owner 门禁。`T21-D29-A01` 以 `main@cba2cec93eaab9a4ae5e7c8bd1c827283d4c784f` 为审计 base，追加开放 Generator compile gate/既有测试，把命令清单变为可测试合同并在目标模块 test/build 前运行 application architecture tests；生成内容、自动产品注册 OUT 边界、API、schema/migration、前端、UI/CSS、发行和暂停 E2E 不变。实现与 result 仍随 D29 一并回写。
