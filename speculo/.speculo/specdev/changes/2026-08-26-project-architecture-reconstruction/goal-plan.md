...
- 第五轮登录/账户基线审计继续发现历史已有的密码显隐、终端逐行动效/`620px/10px`/`1100px` 断点，以及个人中心 `1:3` 身份摘要/资料密码 tabs/确认密码在重构后丢失；这些均属于当前 T-06/`AC-012` 仍保留的页面与密码能力。`T05-D08` 以 implementation/result `0db2125a06b470a0196db53fd7a09d060c11acec`（tree `fd7b29c0069028abb4f9e92ab8f6fc8dae1d89c2`）恢复上述结构和交互，只保留 username/displayName/email/avatar metadata，增加确认密码 Controller 前阻断与头像清空归一化；验证码、旧资料字段、旧头像上传和兼容链零恢复。lint、全 typecheck、九 driver 纯编译、29 files/146 tests、7 boundaries、双 build和全部根静态 Gate 通过，未运行 E2E。
