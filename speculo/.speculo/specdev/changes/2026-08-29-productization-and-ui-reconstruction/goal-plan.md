...
T-20 candidate `2582a4b`（tree `7be9540`）已实现 DEV-20-020：不改产品或 50px 视觉合同，从完整管理员导航中可见的第一 route“用户管理”聚焦后固定 11 次 Tab，让 WebKit 标准焦点遍历把第十二 route“产品示例”滚入可视区，随后 fresh focused query、唯一 0.2 秒同步与唯一 center click；固定 navigation-start/traversal/action 诊断。governance `25c8ec1` 与 source `66d7003` 均为其祖先且无冲突，Vitest 256/256、Node 48/48、lint、diff-check 与 clean tree 已通过；最多 3 次逐个归因的 DEV-20-020 attempt 尚未开始。最后一次旧配额 probe `2f3e87d` / Actions `33539914535` / job `99963330288` 的 `login-navigation` 红灯仍保留。G7 必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
