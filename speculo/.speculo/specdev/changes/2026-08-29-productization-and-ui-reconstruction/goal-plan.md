...
| Workspace records | T-20 latest source `37c6841`、portable candidate `822936e`/tree `d593fb1` 已记录并通过 portable checks；probe `f062e11` 的 DEV-20-014 attempt 2 `login-demo-workspace` 红灯保留，既有 source/candidate、probe、扫描 artifacts 与旧失败候选继续保留 |
| Authorization | local source commits、candidate integration、`main` fast-forward、required runner 本地准备与既有 DEV、DEV-19-001~010 已授权；DEV-20-005/006/008/011 各 3 次 probe 均已用尽；DEV-20-007 已用 2 次且剩余 runner-only attempt 不用于产品修正；DEV-20-014 已用 2/3 次；DEV-20-016 只开放 exact enabled button 中心点单次 mouse click 并沿用最后 1 次配额；其他远程写入/部署/发布/生产迁移/清理未授权 |
...
- G7/T-20 的 hosted macOS 15.7.7 arm64 通道已实测 `System Events` UI elements enabled=`true`。DEV-20-014 attempt 2 再次返回 `login-demo-workspace`，证明同一 exact enabled button 的 focus + `delay 0.2` + Enter 仍未激活 route。DEV-20-016 只从该 button 的 AX position/size 计算中心点并发送一次 mouse click，不改变产品、fixture、调用点或等待上限。
...
T-20 portable candidate `822936e` 与 DEV-20-014 attempt 2 红灯均保留。下一 source/candidate 只实现 DEV-20-016 的 exact button center-point single click，再更新同一 workflow-only probe并使用最后 attempt 3。G7 仍必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`；该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
