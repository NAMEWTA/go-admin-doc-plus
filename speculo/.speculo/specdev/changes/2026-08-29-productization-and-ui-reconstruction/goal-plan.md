...
T-20 candidate `0847ff6`（tree `96f8129`，source `d2d4ed5`）已实现 DEV-20-022，并通过 Vitest 256/256、Node 48/48、Desktop runner 21/21、完整 typecheck、lint、production build/asset scan、native-e2e build、diff-check 与 clean tree；测试产物确认包含 `E2E open Demo`，production 重建再次通过零测试字节扫描。attempt 2 probe `5717707` / Actions `33544806661` / job `99979569584` 的 prebuild 通过，required gate 从 18:44:36Z 至 18:51:53Z 仍固定返回 `navigation-traversal`，证明 hosted Tauri WebView 的屏外 sidebar target 不接受普通 Tab 或 Option-Tab AX focus traversal。DEV-20-022 只在 production scanner 强制排除的 native-e2e entry 增加固定可见 Demo 控件并驱动同一真实 hash router，产品 sidebar 已由 required Web Gate 覆盖；DEV-20-020 尚余最后 1 次配额。G7 必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
