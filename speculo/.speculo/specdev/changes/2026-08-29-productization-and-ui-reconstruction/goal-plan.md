...
T-20 candidate `0847ff6`（tree `96f8129`，source `d2d4ed5`）已实现 DEV-20-022，并通过 Vitest 256/256、Node 48/48、Desktop runner 21/21、完整 typecheck、lint、production build/asset scan、native-e2e build、diff-check 与 clean tree。attempt 3 probe `0d30fd4` / Actions `33547554824` / job `99988721801` 的 prebuild 通过，required gate 从 19:13:18Z 至 19:21:48Z 越过 login navigation、Demo page 与 boundary，首个红灯推进到 `theme-dark-toggle` 的 header AX button unavailable，直接证明 DEV-20-022 有效；DEV-20-020 配额已用尽。DEV-20-023 只新增 production scanner 强制排除的固定测试按钮，由其 exact DOM-click 真实产品主题按钮并保留 aria-label/storage/restart 后置条件，最多 3 次逐项归因配额。G7 必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
