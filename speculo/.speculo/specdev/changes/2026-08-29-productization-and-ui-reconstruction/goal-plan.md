...
T-20 candidate `d1b0d9c`（tree `3d28be1`，source `85d1f14`）已实现 DEV-20-021 并通过 Vitest 256/256、Node 48/48、lint、diff-check 与 clean tree；attempt 1 probe `c955fcb` / Actions `33542592499` / job `99972252465` 的 prebuild 通过，required gate 从 18:23:21Z 至 18:31:57Z 固定返回 `navigation-traversal`。Apple 官方合同说明默认 Tab 不遍历网页按钮；DEV-20-021 已只把固定 11 次循环改为 Option-Tab，其他查询、诊断、delay/click、产品/workflow/timeout 均不变，并沿用 DEV-20-020 剩余 2 次配额。G7 必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
