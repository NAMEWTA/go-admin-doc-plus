...
T-20 portable candidate `1e17579` 已实现 DEV-20-019：沿 exact target 的 `AXParent` 在固定 12 层内找 enclosing `AXScrollArea`，将关联 `AXVerticalScrollBar` 唯一一次设为 `AXMaxValue`，再沿用 fresh query 与现有单次 center click；Node 48/48、Vitest 41/41 files / 256/256 tests、lint、Speculo 0/0、diff-check 与 clean-tree verification 均通过，不增加产品、workflow、click、delay 或 attempt 配额。probe `b3244c0` 的 DEV-20-018 attempt 2 红灯仍保留。下一步更新同一 workflow-only probe，并使用最后 1 次 ordered attempt。G7 仍必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`；该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
