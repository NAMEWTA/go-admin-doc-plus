...
T-20 portable candidate `1e17579` 已实现 DEV-20-019：沿 exact target 的 `AXParent` 在固定 12 层内找 enclosing `AXScrollArea`，将关联 `AXVerticalScrollBar` 唯一一次设为 `AXMaxValue`，再沿用 fresh query 与现有单次 center click；Node 48/48、Vitest 41/41 files / 256/256 tests、lint、Speculo 0/0、diff-check 与 clean-tree verification 均通过。最后一次 ordered attempt 为 probe `2f3e87d` / Actions `33539914535` / job `99963330288`：prebuild 通过，required gate 在 17:56:05Z 至 18:05:04Z 返回 `login-navigation`: `desktop native accessibility button unavailable`，没有 pass marker；DEV-20-017 的 3 次配额已耗尽。G7 仍必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`；任何后续 probe 必须先建立新的具体归因、边界和独立配额，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
