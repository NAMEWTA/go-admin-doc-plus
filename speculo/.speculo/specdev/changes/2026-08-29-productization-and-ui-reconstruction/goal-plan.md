...
| Parent | 当前 `main@c5ff411` 仍未包含 T-20 产品树；最新 candidate 已包含该治理父节点；`4870670` 误合并由 `8e8855b` 前向撤销并保留审计历史，未声称晋升 |
| Tickets | T-01~T-19 均 `done`；T-20 `in_progress`；T-21 `ready` |
| Gate | G0~G6 已通过；G7/T-20 执行中；G8 尚未开启 |
| Workspace records | T-20 latest source `70f2bc2`、portable candidate `d646848`/tree `5215569` 已记录并包含 `main@c5ff411`；probe `10ca36f` 的 DEV-20-007 attempt 2 保留为 `first-setup-recovery-login-workspace` 红灯；既有 source/candidate、probe、扫描 artifacts 与旧失败候选继续保留 |
| Authorization | local source commits、candidate integration、`main` fast-forward、required runner 本地准备与既有 DEV、DEV-19-001~010 已授权；DEV-20-005/006 各 3 次 probe 均已用尽；DEV-20-007 已用 2 次且剩余 runner-only attempt 不用于产品修正；DEV-20-008 精确开放恢复按钮 Session 清理及最多 3 次同一 workflow-only probe branch macOS dispatch；其他远程写入/部署/发布/生产迁移/清理未授权 |
...
- G7/T-20 的 hosted macOS 15.7.7 arm64 通道已实测 `System Events` UI elements enabled=`true`。DEV-20-005 三次 attempt 依次暴露 Go module 预热、受控诊断遮蔽与 `tauri-utils 2.9.3` `dataStoreIdentifier` codegen 类型错误；DEV-20-006 三次 attempt 将红灯缩小到恢复态后的三步。DEV-20-007 attempt 1 `33506930543` 精确证明 snapshot restore 与 continue click 通过，attempt 2 `33508881242` 固定分类为 authenticated workspace，证明 recovery 按钮未清除当前进程 Session。DEV-20-008 只修正该 AC-005 产品缺口，既有等待与安全日志边界不变。
