...
T-20 candidate `c8d90d7`（tree `9e6c4b1`，source `6d611a5`）已实现 DEV-20-026，并通过 Vitest 256/256、Node 49/49、Desktop runner 22/22、完整 typecheck、lint、native-e2e 恢复标记检查、production build/asset scan、diff-check 与 clean tree。DEV-20-023 attempt 3 probe `c053fb8` / Actions `33553644665` / job `100009067137` 的 prebuild 通过，required gate 从 20:16:31Z 至 20:25:47Z 精确返回 `permission-restored`，证明 disable/denied/hidden/enable 均已越过；真实 router 在权限关闭时已停留 `/forbidden`，原 runner 未导航便错误等待 Demo 自动回跳。DEV-20-026 只在 test-only 恢复 action 成功后提供固定握手，再复用既有 Demo 控件走真实 router并保留产品后置条件，尚余 3 次新配额。G7 必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
