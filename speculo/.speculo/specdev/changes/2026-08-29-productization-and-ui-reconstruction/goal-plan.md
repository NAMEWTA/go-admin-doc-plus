...
| Parent | 当前 `main@f2e7a6b` 仍未包含 T-20 产品树；最新 candidate 包含此前治理父节点 `96690de`；`4870670` 误合并由 `8e8855b` 前向撤销并保留审计历史，未声称晋升 |
| Tickets | T-01~T-19 均 `done`；T-20 `in_progress`；T-21 `ready` |
| Gate | G0~G6 已通过；G7/T-20 执行中；G8 尚未开启 |
| Workspace records | T-20 latest source `526f7f0`、portable candidate `83544da`/tree `116e3ed` 已记录；candidate 包含 `main@96690de` 与 source；probe `e5a9496` 的 DEV-20-006 attempt 2 保留为同一 `first-setup-recovery-state` 红灯；既有 source/candidate、probe、扫描 artifacts 与旧失败候选继续保留 |
