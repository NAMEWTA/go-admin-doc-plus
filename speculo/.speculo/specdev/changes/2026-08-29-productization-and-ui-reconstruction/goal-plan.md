...
T-20 portable candidate `2d302d2` 已实现 DEV-20-018：在唯一 AXScrollToVisible 后 fresh query 同一 exact enabled button，再用刷新引用执行既有 focus/delay/center click；Node 48/48、Vitest 41/41 files / 256/256 tests、lint、Speculo 0/0、diff-check 与 clean-tree verification 均通过，未增加 action、click、delay 或 attempt 配额。probe `acd5a91` 的 DEV-20-017 attempt 1 红灯仍保留。下一步更新同一 workflow-only probe，并使用剩余 2 次中的下一次 ordered attempt。G7 仍必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`；该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
