...
| Workspace records | T-20 latest source `b284047`、portable candidate `565a959`/tree `0c03a96` 已记录并通过 portable checks；probe `20c6b63` 的 DEV-20-014 attempt 1 `login-demo-workspace` 红灯保留，既有 source/candidate、probe、扫描 artifacts 与旧失败候选继续保留 |
| Authorization | local source commits、candidate integration、`main` fast-forward、required runner 本地准备与既有 DEV、DEV-19-001~010 已授权；DEV-20-005/006/008/011 各 3 次 probe 均已用尽；DEV-20-007 已用 2 次且剩余 runner-only attempt 不用于产品修正；DEV-20-014 已用 1/3 次；DEV-20-015 只开放 exact button focus 后 `delay 0.2` 再 Enter 并沿用剩余 2 次配额；其他远程写入/部署/发布/生产迁移/清理未授权 |
...
- G7/T-20 的 hosted macOS 15.7.7 arm64 通道已实测 `System Events` UI elements enabled=`true`。DEV-20-014 attempt 1 越过 ordinary first setup 后再次返回 `login-demo-workspace`，证明前一 setup 红灯非确定且 focus 后立即 Enter 仍未激活 route。DEV-20-015 只在同一 exact enabled button focus 后加入已通过 native submit 使用的 `delay 0.2` 再 Enter，不改变产品、fixture、调用点或等待上限。
