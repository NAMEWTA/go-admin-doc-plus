...
T-20 portable candidate `1e17579` 已实现 DEV-20-019；最后一次旧配额 attempt 为 probe `2f3e87d` / Actions `33539914535` / job `99963330288`：prebuild 通过，required gate 在 17:56:05Z 至 18:05:04Z 返回 `login-navigation`: `desktop native accessibility button unavailable`，没有 pass marker，DEV-20-017 的 3 次配额已耗尽。DEV-20-020 已独立授权：不改产品或 50px 视觉合同，从完整管理员导航中可见的第一 route“用户管理”聚焦后固定 11 次 Tab，让 WebKit 标准焦点遍历把第十二 route“产品示例”滚入可视区，随后 fresh query/唯一 0.2 秒同步/唯一 center click；固定 navigation-start/traversal/action 诊断且最多 3 次逐个归因 attempt。G7 仍必须在真实 macOS execution 得到 `DESKTOP_NATIVE_E2E_PASS runtime=tauri-native profile=sqlite skipped=0`，该 marker 出现前 candidate 不晋升。所有既有 source/candidate、probe、未跟踪扫描 artifacts 与保护 stash均保留。
