...
- [x] `AC-023`：OpenAPI 3.1 可确定性生成 Go strict transport 和新 Workspace TS client，clean tree 无漂移。
- [x] `AC-025`：公共负向响应只使用已声明错误类别且不泄露内部信息。
- [x] T02-D04 修正与验证矩阵记录到 `<Path>{roots.state}/specdev/changes/2026-08-26-project-architecture-reconstruction/evidence/T-02.md</Path>`。
- [x] 修改未超出 `writable_paths`，共享路径仅由 T-02 修改。
- [x] 形成新的非空 source checkpoint，并记录新的 candidate/result SHA。
