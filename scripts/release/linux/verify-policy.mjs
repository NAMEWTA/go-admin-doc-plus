...
export async function verifyRepository(repository) {
  ...
  assert.match(server, /pnpm --dir go-admin-plus-ui install --frozen-lockfile --offline --ignore-scripts/)
  assert.match(server, /git init --quiet/)
  assert.match(server, /--chown=10001:10001 \/opt\/go-admin-plus\/repository/)
  assert.match(build, /release\/linux\/Containerfile\.server/)
  assert.match(build, /release\/linux\/Containerfile\.web/)
  assert.match(failure, /missing-release-skeleton/)
