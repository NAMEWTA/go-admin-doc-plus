...
test('detects removed paths and active compatibility references', () => {
  ...
  mkdirSync(join(root, 'go-admin-plus/internal/modules/files/migrations/0020-capacity'), { recursive: true })
  mkdirSync(join(root, 'go-admin-plus/internal/modules/demo'), { recursive: true })
  mkdirSync(join(root, 'go-admin-plus/internal/platform/logging'), { recursive: true })
  ...
  writeFileSync(join(root, 'go-admin-plus/internal/modules/files/migrations/0020-capacity/provider_test.go'), 'const forbidden = "tenant"\n')
  writeFileSync(join(root, 'go-admin-plus/internal/modules/demo/service.go'), 'import "go-admin/internal/platform/database"\nimport "gorm.io/gorm"\nconst tokenKind = "jwt"\nconst database = "mysql"\n')
  writeFileSync(join(root, 'go-admin-plus/internal/platform/logging/redaction.go'), 'const sensitiveScheme = "mysql://"\n')
  ...
  assert.ok(failures.some(message => message === 'MySQL remains in go-admin-plus/internal/modules/demo/service.go'))
  ...
  assert.ok(!failures.some(message => message.includes('go-admin-plus/internal/modules/files/migrations/0020-capacity/provider_test.go')))
  assert.ok(!failures.some(message => message.includes('go-admin-plus/internal/platform/logging/redaction.go')))
